...
public:
    ...
    void init(int start){//O((E+V)logV)
        dist.assign(N, INF);
        priority_queue<P, vector<P>, greater<P>> to_do;//（仮距離，頂点）のデータを格納
        
        dist[start] = 0;
        to_do.emplace(0, start);//pushとほとんど同じだけど，Pのコンストラクタ引数を直接渡せて便利
        ...
        while(!to_do.empty()){
            ...
            if(tmp_dist_v != dist[v]) continue;//queueには同じ頂点のデータで最終更新より前の仮距離を持つものも入ってるかもしれないので，それを無視．これがないとO(V^2)
